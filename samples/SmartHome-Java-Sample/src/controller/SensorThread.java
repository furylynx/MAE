...
public class SensorThread implements Runnable {
	 ...
	 * 
	 * @param info
	 *            The sensor info object to which the data will be written.
	 * @param startingTimeStamp
	 *            The starting time stamp.
	 * @param tolerance
	 *            The tolerance to be set. Tolerance is given in beats (1 beat
	 *            is 200ms).
	 * @param bodyParts
	 *            The body parts to be used.
	 * @param columnDefinitions
	 *            The column definitions to be used.
	 * @param sequencesToRegister
	 *            The sequence to be registered.
	 */
	public SensorThread(SensorInfo info, long startingTimeStamp,
			double tolerance, BoneVector bodyParts,
			ColumnDefinitionVector columnDefinitions,
			List<LabanSequence> sequencesToRegister) {
		this.setInfo(info);
		this.running = true;

		niteController = new NiteController(info.getDeviceInfo(),
				"SamplesConfig.xml");

		// framerate is assumed to be 30fps, pose buffer is set to 300 frames =
		// 10 s
		movementController = new WrappedMovementController(bodyParts,
