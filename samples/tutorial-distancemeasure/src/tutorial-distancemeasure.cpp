...
int main()
{
    try
    {
        ...
        if (nullptr != sequence2 && nullptr != sequence1 && nullptr != target_sequence)
        {
            ...
            std::size_t window = 0;
            std::shared_ptr<mae::math::i_distance_measure<std::vector<std::shared_ptr<mae::fl::laban::i_movement> > > > distance_measure = std::make_shared<mae::math::lcs_distance<std::shared_ptr<mae::fl::laban::i_movement> > >(movement_comparator, true);
            ...
            double similarity = comparator->similarity(target_sequence, sequence1);//target_comparator.similarity(target_sequence, sequence1);//
