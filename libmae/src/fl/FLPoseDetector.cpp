				...
				for (int dir = FLD_INVALID + 1; dir != FLD_SIZE; dir++)
				{
					if (dir == FLD_P_M)
					{
						if (k % 3 == 0)
						{
							...
							result->setDistance(joint_id, dir, 180 - skeleton->getJoint((int)flj_ext_bones[k+2])->getPhi());
						}
						else
						{
					...
					{
						result->setDistance(joint_id, dir, skeleton->getJoint(joint_id)->getPhi());
					}
					else if (dir == FLD_R_M)
					{
						result->setDistance(joint_id, dir, 180 - skeleton->getJoint(joint_id)->getPhi());
					}
					else
					{
							...
							for (int j = 0; j < 8; j++)
							{
								double dist = std::sqrt(
										std::pow(((i + 1) * 45) - skeleton->getJoint(joint_id)->getPhi(), 2)
												+ std::pow((j * 45) - skeleton->getJoint(joint_id)->getTheta(), 2));
