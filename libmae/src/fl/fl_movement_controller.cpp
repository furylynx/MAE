...
namespace mae {
	namespace fl {
		...
		fl_movement_controller::fl_movement_controller() : movement_controller(std::shared_ptr<fl_pose_detector>(new fl_pose_detector()),std::shared_ptr<laban_sequence_generator>(new laban_sequence_generator()), get_flj_ids())
		{
			this->skel_ctrl = std::shared_ptr<fl_skeleton_controller>(new fl_skeleton_controller());
		}

		fl_movement_controller::fl_movement_controller(std::vector<int> bodyParts) : movement_controller(std::shared_ptr<fl_pose_detector>(new fl_pose_detector()),std::shared_ptr<laban_sequence_generator>(new laban_sequence_generator()), bodyParts)
		{
			this->skel_ctrl = std::shared_ptr<fl_skeleton_controller>(new fl_skeleton_controller());
		}
