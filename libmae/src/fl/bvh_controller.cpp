	...
	namespace fl
	{
		...
		std::string bvh_controller::bvh_str(std::vector<std::shared_ptr<general_skeleton> > data, double framerate)
		{
			...
			for (unsigned int i = 0; i < ext.size(); i++)
			{
				for (unsigned int j = 0; j < ((std::vector<int>) ext[i]).size(); j++)
				{
					...
					sstr << indent.str() << "OFFSET" << " " << std::setprecision(2)
							<< ((std::shared_ptr<general_skeleton>) data[0])->get_joint((int) ext[i][j])->get_x() - p_x
							<< " " << std::setprecision(2)
							<< ((std::shared_ptr<general_skeleton>) data[0])->get_joint((int) ext[i][j])->get_y() - p_y
							<< " " << std::setprecision(2)
							<< ((std::shared_ptr<general_skeleton>) data[0])->get_joint((int) ext[i][j])->get_z() - p_z
							<< std::endl;
			...
			for (unsigned int skel_id = 1; skel_id < data.size(); skel_id++)
			{
				...
				{
					//offset for each joint
					cv::Vec3d p_vec = fl::math::jointToVec(
					...
					for (unsigned int j = 1; j < ((std::vector<int>) ext[i]).size(); j++)
					{
						...
						cv::Vec3d cur_vec = fl::math::jointToVec(
						...
						cv::Vec3d old_vec = fl::math::jointToVec(
						...
						old_vec = fl::math::matrix_mul(rot_mat, old_vec);

						//rotation
						cv::Vec3d angles = fl::math::rotation_angles_zxy(old_vec, new_vec);
						sstr << std::setprecision(2) << fl::math::radToDeg(angles[0]) << " " << std::setprecision(2)
								<< fl::math::radToDeg(angles[1]) << " " << std::setprecision(2)
								<< fl::math::radToDeg(angles[2]) << " ";

						//update rotation matrix
						rot_mat *= fl::math::matrix_rot_zxy(angles[0], angles[1], angles[2]);
		...
		void bvh_controller::bvh_file(std::vector<std::shared_ptr<general_skeleton> > data, std::string filename)
		{
		...
		void bvh_controller::bvh_file(std::shared_ptr<general_skeleton> data, std::string filename)
		{
		...
		std::vector<std::shared_ptr<general_skeleton> > bvh_controller::read_bvh(std::string bvh_str)
		{
			//frame time
...
//				}
			...
			for (int i = 0; i < frames; i++)
			{
				...
				for (unsigned int j = 0; j < joint_sequence.size(); j++)
				{
									...
									!= joint_end_effector.end())
					{
						...
						//offset
						cv::Vec3d orig_offset = fl::math::jointToVec(offset_skel->get_joint((int) joint_sequence[j]))
								- fl::math::jointToVec(offset_skel->get_joint((int) joint_parent[joint_sequence[j]]));

						cv::Vec3d new_offset = fl::math::matrix_mul(
								(cv::Mat) joint_rot_mat[joint_parent[joint_sequence[j]]], orig_offset);

						cv::Vec3d parent_pos = fl::math::jointToVec(
								next_skel->get_joint((int) joint_parent[joint_sequence[j]]));

						next_skel->set_joint((int) joint_sequence[j], fl::math::vecToJoint(new_offset + parent_pos));
					}
					else
					{
						...
						if (channels[3] != -1)
						{
							...
							rot_mats_tmp_vec.push_back(fl::math::matrix_rot_z(fl::math::degToRad(rot_zeta)));
						}
						...
						if (channels[4] != -1)
						{
							...
							{
								rot_mats_tmp_vec.push_front(fl::math::matrix_rot_x(fl::math::degToRad(rot_xi)));
							}
							else
							{
								rot_mats_tmp_vec.push_back(fl::math::matrix_rot_x(fl::math::degToRad(rot_xi)));
							}
						}
						...
						if (channels[5] != -1)
						{
							...
							{
								rot_mats_tmp_vec.push_front(fl::math::matrix_rot_y(fl::math::degToRad(rot_ypsilon)));
							}
							else if (channels[5] > channels[3] && channels[5] > channels[4])
							{
								rot_mats_tmp_vec.push_back(fl::math::matrix_rot_y(fl::math::degToRad(rot_ypsilon)));
							}
							else
							{
								...
								rot_mats_tmp_vec.insert(iterator,
										fl::math::matrix_rot_y(fl::math::degToRad(rot_ypsilon)));
							}
						}
						...
						{
							//handle root element
							cv::Vec3d orig_offset = fl::math::jointToVec(
									offset_skel->get_joint((int) joint_sequence[j]));
							next_skel->set_joint((int) joint_sequence[j],
									fl::math::vecToJoint(orig_offset + motion_offset));
						}
						else
						{
							...
							cv::Vec3d orig_offset = fl::math::jointToVec(
									offset_skel->get_joint((int) joint_sequence[j]))
									- fl::math::jointToVec(
											offset_skel->get_joint((int) joint_parent[joint_sequence[j]]));
							cv::Vec3d new_offset = orig_offset;
							if (j != 0)
							{
								new_offset = fl::math::matrix_mul(
										(cv::Mat) joint_rot_mat[joint_parent[joint_sequence[j]]], orig_offset);
							}
							...
							next_skel->set_joint((int) joint_sequence[j],
									fl::math::vecToJoint(new_offset + parent_pos + motion_offset));
						}
		...
		std::vector<std::shared_ptr<general_skeleton> > bvh_controller::read_bvh_file(std::string filename)
		{
		...
		std::vector<int> bvh_controller::parse_channels(std::string& value, unsigned int pos)
		{
			...
			for (unsigned int i = 0; i < str_channels.size(); i++)
			{
				...
				else if (((std::string) str_channels[i]).compare(bvh_controller::Ypos) == 0)
				{
					el_channels[1] = i;
				}
				else if (((std::string) str_channels[i]).compare(bvh_controller::Zpos) == 0)
				{
					el_channels[2] = i;
				}
				else if (((std::string) str_channels[i]).compare(bvh_controller::Zrot) == 0)
				{
					el_channels[3] = i;
				}
				else if (((std::string) str_channels[i]).compare(bvh_controller::Xrot) == 0)
				{
					el_channels[4] = i;
				}
				else if (((std::string) str_channels[i]).compare(bvh_controller::Yrot) == 0)
				{
					el_channels[5] = i;
				}
			}
				...
				std::shared_ptr<general_joint> parent_joint)
		{
			...
			cv::Vec3d el_position = fl::math::stdVecToVec3d(offset) + fl::math::jointToVec(parent_joint);

			return fl::math::vecToJoint(el_position);
		}
