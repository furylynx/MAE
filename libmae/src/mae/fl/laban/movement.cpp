    ...
    namespace fl
    {
        namespace laban
        {
            ...
            movement::movement(int column, unsigned int measure, double beat, double duration,
                               std::shared_ptr<mv::i_symbol> symbol, bool hold,
                    ...
                    return (a_mov->get_symbol()->equals(symbol_) && a_mov->get_hold() == hold_
                            && ((a_mov->get_pre_sign() == nullptr && pre_sign_ == nullptr)
                                || (a_mov->get_pre_sign() != nullptr &&
                                    a_mov->get_pre_sign()->equals(pre_sign_))));
                }
                else
                {
                    return false;
            ...
            bool movement::all_types_equal(std::shared_ptr<i_movement> a) const
            {
                if (std::shared_ptr<movement> a_mov = std::dynamic_pointer_cast<movement>(a))
                {
                    return (a_mov->get_symbol()->all_types_equal(symbol_) &&
                            ((a_mov->get_pre_sign() == nullptr && pre_sign_ == nullptr)
                             || (a_mov->get_pre_sign() != nullptr &&
                                 a_mov->get_pre_sign()->all_types_equal(pre_sign_))));
                }
                else
                {
                    return false;
            ...
            std::string movement::xml(unsigned int indent, std::string namesp) const
            {
                ...
                sstr << indent_stream.str() << "\t" << "<" << ns << "column>" << column_ << "</" << ns
                     << "column>"
                     << std::endl;
                sstr << indent_stream.str() << "\t" << "<" << ns << "measure>" << measure_ << "</" << ns
                     << "measure>"
                     << std::endl;
                sstr << indent_stream.str() << "\t" << "<" << ns << "beat>" << beat_ << "</" << ns
                     << "beat>"
                     << std::endl;
            ...
            std::string
            movement::svg(unsigned int im_width, unsigned int im_height, unsigned int max_column,
            ...
            std::string movement::svg(draw_laban_rect rect, svg_style style) const
            {
                ...
                double draw_x_pos = (rect.get_im_width() / 2.0) +
                                    ((column_ - (mae::math::math::sign(column_) * 0.5) - 0.25) *
                                     column_width);
                ...
                if (measure_ != 0)
                {
                    draw_y_pos = rect.get_im_height() * (0.85 - 0.01) -
                                 (measure_ * rect.get_beats_per_measure() + beat_
                ...
                {
                    draw_y_pos =
                            rect.get_im_height() * (0.85) - (measure_ * rect.get_beats_per_measure() + beat_
                ...
                sstr << symbol_->svg(identifier, draw_rect(draw_x_pos, draw_y_pos, draw_w, draw_h),
                                     (column_ < 0), style);
                ...
                if (hold_)
                {
                    //draw circle
                    sstr << "\t\t<path" << std::endl;
                    sstr << "\t\t\td=\"m " << draw_x_pos + draw_w / 2.0 + draw_hold_h / 2.0 << ","
                         << draw_hold_y + draw_hold_h / 2.0 << " a " << draw_hold_h / 2.0 << ","
                         << draw_hold_h / 2.0 << " 0 1 1 -" << draw_hold_h << ",0 " << draw_hold_h / 2.0
                         << "," << draw_hold_h / 2.0 << " 0 1 1 " << draw_hold_h << ",0 z\"" << std::endl;
                    sstr << "\t\t\tid=\"" << identifier << "-hold\"" << std::endl;
                    sstr << "\t\t\tstyle=\"fill:#" << style.get_fill_color() << ";fill-opacity:1;stroke:#"
                         << style.get_draw_color() << ";stroke-width:" << style.get_stroke_width()
                ...
                if (pre_sign_ != nullptr)
                {
                    ...
                    sstr << pre_sign_->svg(identifier,
                                           draw_rect(draw_x_pos, draw_ps_y, draw_ps_h, draw_ps_h), false,
                                           style);
                }
            ...
            std::shared_ptr<i_movement>
            movement::recreate(std::map<int, int> column_mapping, unsigned int measure,
            ...
            std::string movement::str() const
            {
                ...
                sstr << "movement: [c" << column_ << ",m" << measure_ << ",b" << beat_ << ",d" << duration_
                     << ",h"
                     << hold_ << "] " << symbol_;
