	...
	namespace fl
	{
		namespace laban
		{
			namespace mv
			{
				...
				std::string turn_symbol::svg(std::string identifier, draw_rect rect, bool left, svg_style style) const
				{
					...
					if (direction_ == e_turn_direction::CLOCKWISE || direction_ == e_turn_direction::ANY)
					{
						...
						sstr << "\t\t\tstyle=\"";
						sstr << "fill:#ffffff;fill-opacity:1;stroke:#" << style.get_draw_color() << ";stroke-width:" << style.get_stroke_width() << "pt;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none";
						sstr << "\" />" << std::endl;
					}
