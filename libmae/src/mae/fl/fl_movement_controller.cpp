...
namespace mae
{
	namespace fl
	{
		...
		fl_movement_controller::fl_movement_controller()
				: movement_controller(std::shared_ptr<fl_pose_detector>(new fl_pose_detector()),
						std::shared_ptr<laban::laban_sequence_generator>(new laban::laban_sequence_generator()),
						bone::default_bones())
		{
		...
		fl_movement_controller::fl_movement_controller(std::vector<bone> body_parts,
				...
				: movement_controller(std::shared_ptr<fl_pose_detector>(new fl_pose_detector()),
						std::shared_ptr<laban::laban_sequence_generator>(new laban::laban_sequence_generator(column_definitions)),
						body_parts)
		{
			this->skel_ctrl = std::shared_ptr<fl_skeleton_controller>(new fl_skeleton_controller());
		}
